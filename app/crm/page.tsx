...
export default function CRM() {
  ...
  React.useEffect(() => {
    async function load() {
      try {
        ...
        setAllProperties(propsData.length > 0 ? propsData : []);
        
        // Use mock agents if none found in DB
        const finalAgentsData = agentsData.length > 0 ? agentsData : [
          { id: '1', full_name: 'Daniela Ruiz', role: 'Agente Lead', license_id: 'AMPI 4782' },
          { id: '2', full_name: 'Carlos Méndez', role: 'Agente Senior', license_id: 'AMPI 9123' },
          { id: '3', full_name: 'Ana García', role: 'Agente Junior', license_id: 'AMPI 8842' }
        ];

        // Map DB agents to TeamMember interface
        const mappedAgents: TeamMember[] = finalAgentsData.map((a: any, i: number) => ({
          name: a.full_name,
          role: a.role === 'agent' ? 'Agente' : a.role,
          deals: 15 - i,
          pct: 95 - i * 4,
          init: a.full_name.split(' ').map((n: string) => n[0]).join('')
        }));
  ...
  // KPIs
  const kpis: KPI[] = [
    { lbl: "Propiedades activas", val: allProperties.length.toString(), delta: "+12", icon: "building", tone: "orange" },
  ...
  // Active listings
  const listings = allProperties.slice(0, 5).map((p, i) => ({
    ...p,
    views: 234 + i * 87,
    leads: 12 + i * 4,
    offers: i,
    ...
    <div className="crm-shell">
      ...
      <div className="crm-layout">
        ...
        <aside className="crm-side glass">
          <div className="crm-side-logo" style={{cursor:"pointer"}} onClick={() => router.push("/")}>
          ...
          <div className="crm-side-bottom">
            ...
            <button className="crm-side-btn" title="Salir" onClick={() => router.push("/")}><Icon name="arrow-left" size={18}/></button>
          </div>
        </aside>
